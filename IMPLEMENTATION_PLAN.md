...
| **p2.6-d1r-run-N+1 (2026-05-18 this loop)** | **n/a (source-only)** | **n/a** | ✅ D1-reborn expression-level emitters landed: 4 new `*ToGoDirect` emitters (infix, prefix, call, index) + 8 allowlist additions (mangle, positionGetLine/Column/ToString, numberLiteralEvaluate, getStringLiteralValue, evaluateStringLiteral, getBooleanLiteralValue, evaluateBooleanLiteral); verify.sh 5/5 ✅, test.sh ✅ |

**Headline:** committed HEAD binary passes verify.sh 5/5. Phase-2 self-host bench is 0.91× vs phase0 — still off the 10× target. D1-reborn expression-level emitters shipped this loop (source-only; 9 leaf defs now opted into direct Go emit — `nullLiteralEvaluate` from Run N plus `mangle`, `positionGetLine`, `positionGetColumn`, `positionToString`, `numberLiteralEvaluate`, `getStringLiteralValue`, `evaluateStringLiteral`, `getBooleanLiteralValue`, `evaluateBooleanLiteral`). The new floor is **p2-stage2-regression-fix = 1m17.810s**.

**🎯 Next-loop start here:** `P2.6 → D1-reborn → Run N+2` (statement-level direct emitters + library globals plumbing) below. Every other P2.5/P2.6 source-level win is invisible to `bench.sh` until the committed bridge is replaced, and the bridge can't be replaced until D1-reborn closes the 5× stage2 perf regression (multi-run roadmap: N+2 → N+3). P3/P4/P5 stay queued behind that gate.
...
- [ ] **Run N+2: Statement-level direct emitters + library globals + flip predicate.**
  - Add `assignmentStatementToGoDirect`, `variableDeclarationToGoDirect`, `ifStatementToGoDirect`, `whileStatementToGoDirect`, `returnStatementToGoDirect` (already done as `ReturnStatement_toGoDirect`), `indexAssignmentToGoDirect`, `arrayLiteralToGoDirect`, `mapLiteralToGoDirect`. After this most parser helpers should be direct-emittable.
  - **Library globals plumbing.** `programToGoPhase2` must emit two new emit-pass artefacts: (a) package-scope `var ij_<libname> Value` declarations for every `resolvedLibraryGlobals` entry, and (b) `ij_<libname> = ctx.Get("<libname>")` inside main() right after `registerLibraryFunctions(ctx)`. This unlocks direct-emit'd defs that reference library functions (e.g. `string`, `len`, `chr`, `ord`, `push`, `puts`) — currently a hard gate per Run N+1 limitation noted above.
