...
- [ ] **P-VM.2 — full node-kind coverage** (arrays/maps/index/index-assign/staticCall/closures+upvalues/nested-block lets via sub-scope slots or bail-narrowing). `verify.sh` 5/5 with `IJ_VM=1`. Goal: drive interpreter.s's own escape count 269→~0 and compile the 12 holdout bodies, so P-VM.3 (default flip) is meaningful.
