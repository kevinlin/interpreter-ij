...
**Headline:** committed HEAD binary passes verify.sh 5/5. Phase-2 self-host bench is 0.91× vs phase0 — still off the 10× target. D1-reborn Run N+2 shipped this loop (source-only; 142 leaf+stmt defs now direct-emit via canDirectEmit coverage check). Stage2 selfhost regression CLOSED FROM 5× → 3.5× over stage1 (`7m25s → 4m32s`); bridge replace still gated until <1m30s. The new floor at the bench level is unchanged: **p2-stage2-regression-fix = 1m17.810s** (committed bridge runs the bench, source-only wins remain invisible).

**🎯 Next-loop start here:** `P2.6 → D1-reborn → Run N+3` (close remaining stage2 regression — 84 promoted defs still on the Node-tree+eval path, plus FunctionCommand.Execute overhead in indirect callers — then replace committed bridge) below. P3/P4/P5 stay queued behind that gate.
...
- [ ] **Run N+3: Close the remaining stage2 regression + replace the committed bridge.**
  - 84 promoted defs are still on the Node-tree+eval path. Candidates to investigate:
    - Defs whose body references a top-level user `let` (e.g. `keywords`, `currentToken`). Add `resolvedRootGlobals` plumbing (emit `var ij_<letname> Value` + populate inside programNode eval after the var is declared) so those refs become valid Go.
    - Defs whose body uses a static def as a VALUE (not as a callee). Emit `var ij_<defname> Value` populated from `Value{tag:tFunc, cmd: NewFunctionCommand(ij_<n>_body)}` after registration so bare refs compile.
    - Override-pattern defs (`let oldX = X; def X(...) { oldX(...) }`) — counts=1 gate excludes these. May be safely promotable if we detect the pattern (single let-then-def shadow with matching shape) and treat as a single binding.
  - Verify stage1 ≡ stage2 ≡ stage3 byte-identical (true fixed-point at the binary level — Run N+2 already proves stage2 ≡ stage3; Run N+3 must demonstrate stage1 ≡ stage2 once the bridge is replaced).
