...
**Headline:** Run N+5 positional-arg calling convention is wiring-complete + verified (5/5, arity-tolerant). The expected perf payoff did not materialise. **stage2 selfhost stays at 4m15s vs stage1 1m45s.** The dominant cost under selfhost is the `MapValue["evaluate"]` closure dispatch path, NOT direct-by-name static calls. Every AST node in `interpreter.s` carries a `node["evaluate"]` closure (set inside `make*` factories); calls to it route `value-Get → FunctionCommand.Execute → evalFuncDecl.func1 → eval(body, local)`. D1-reborn + N+5 do nothing for that path. Bridge replace stays blocked.

**🎯 Next-loop start here:** The structural fix is to direct-emit `MapValue["evaluate"]` closure bodies. Two sub-paths to explore (pick one this loop):

1. **Specialise `node["evaluate"](node, ctx)` call sites.** Recognise the pattern `<expr>["evaluate"](<expr>, <ctx>)` in the codegen; if `<expr>` resolves to a known-shape AST-node MapValue (i.e. a `makeXxx()` factory output), look up the closure's compiled name (the lambda inside the factory) and emit a direct call.
2. **Hoist closure bodies out of `makeXxx` factories.** Each `makeFooStatement(...)` currently does `def evaluate(self, context) { ... }; node["evaluate"] = evaluate;`. Move the closure body to a top-level `def fooStatement_evaluate(self, context) { ... }` and store `node["evaluate"] = fooStatement_evaluate` (= a `Value{tag:tFunc}` referencing a static def, not a per-`makeFoo`-call closure). Static-def reference closes over no upvalues → safe. Then promote `fooStatement_evaluate` via D1-reborn → direct-emit body. Call sites `node["evaluate"](node, ctx)` still go through Execute, but the closure body itself is now direct-Go-statements with no `eval()` recursion. Need: source refactor in `make*` factories + verifier that no closure references an outer-scope `let` that would break hoisting.

Path (2) is structurally cleaner and matches the OLD bridge's emit shape (it inlines `evaluate` bodies as direct Go because there is no `node["evaluate"] = closure` capture in the OLD-emit interpreter — see `/tmp/p2_6/app_s1.go`). Path (1) preserves source but adds emit-side heuristics.

Capture stage2 pprof on a quiet machine first (P2.6 backlog item, now critical-path) to confirm `evalFuncDecl.func1` is still the top frame under the N+5 emit shape.

**Current bridge architecture (post Run N+5, 2026-05-27):**
...
| stage1 (committed → new src) | NEW-style: 226 ij_*_impl + nkStaticCall + 214 direct-emit bodies (OLD-bridge produced via NEW source) | 4.92 MB | 1m45.1s | parity with committed |
| stage2 (stage1 → new src) | NEW-style: positional-arg ij_*_impl + _impl_wrapper + nkStaticCall via wrapper, true fixed-point with stage3 | 5.70 MB | **4m15.8s** | **2.4× slow; bridge replace gated** |

Stage1's compiled-in `eval`/`Execute` machinery is the OLD bridge's shape applied to the new source; stage2's is the NEW shape. Stage2 ≡ stage3 byte-identical (Run N+5 fixed-point holds). Diagnosis (carry from Run N+3): the dominant cost under selfhost is the `MapValue["evaluate"]` closure path (`Execute → evalFuncDecl.func1 → eval(body, local)`); user-level `make*` factory closures still go through that path even after Run N+5's positional-arg conv. Run N+6 (closure-body hoist / call-site specialisation) is the next lever.
