...
**Next-loop target = D2-reborn THEN bridge replace THEN P3.** P2's stage2-regression is now resolved (this loop). New stage2 builds correctly, bridge tests + selfhosted sample.s + true source-level fixed-point all pass on stage2. **However, replacing the committed binary with the new stage2 is now blocked behind D2-reborn:** the committed bridge was built from the pre-cleanup `ac2e6f3`-era source that emitted ~222 D2 fast-path `ij_<name>_impl` static Go functions — those make the IJ tree-walker (which `selfhosted_interpreter.sh` and `scripts/interpreter.sh` exercise) ~5× faster than the new emit. Manual measurement: `selfhosted_interpreter.sh sample.s` under the new stage2 = **7m02s** vs **1m17s** under the committed bridge. Replacing today is a **net regression** even with all P2.5 fast-paths active.

Phase 0 ✅ • Phase 1 ✅ committed • Phase 2 ✅ wired structurally; semantically incomplete (resolver annotations dead) • **Phase 2.5 ✅ shipped (source only; visible gain blocked on D2-reborn → bridge replace)** • Phase 3 ⬜ • Phase 4 ⬜

**P2 status (post stage2-regression fix):** Phase-2 emit pipeline is reproducible at the source level (stage2≡stage3 byte-identical, verified this loop with the actual fresh-build bridge installed). The IJ tree-walker on stage2 now correctly handles user scripts with multi-statement bodies, scalar `let`s, scalar return values flowing through the IJ-level evaluate loop, etc. The remaining gating constraint on committed-binary-replace shifted from "stage2 runtime is broken" to "stage2 runtime is correct but ~5× slower for tree-walker workloads because D2-reborn hasn't shipped." See P2.6 below.
...
**P2 carried forward (still BLOCKER for replacing the committed bridge — root cause shifted):**

- [ ] **Re-build D2 fast-path emit (D2-reborn).** New blocker for committed-binary-replace. The new emit is correct but ~5× slower than the OLD bridge for IJ tree-walker workloads (`scripts/interpreter.sh`, `selfhosted_interpreter.sh`). Manual measurement this loop: `time (echo hi | bash scripts/selfhosted_interpreter.sh src/sample.s)` under stage2-as-bridge = **7m02s** (vs **1m17.810s** under the committed bridge). The committed bridge was built from `ac2e6f3`-era source that emitted ~222 `NewStaticFunctionCommand`-wrapped `ij_<name>_impl` direct Go functions (per `grep -c NewStaticFunctionCommand` on the OLD bridge's app.go); the new emit only produces 2. Every IJ tree-walker call (`node["evaluate"](node, ctx)`, `interpreter["evaluate"](interpreter)`, `program["addStatement"](program, stmt)`, …) goes through `evalCall → FunctionCommand.Execute → executeFunc` instead of a direct Go function call, which is the source of the slowdown. Until D2 emission is reborn (most likely as a Phase-2.6 task that lifts `resolvedIsStatic` annotations into the emit pipeline), **DO NOT replace the committed bridge** — replacement is a net regression today. Track the existing `analyzeIsStatic`/`resolvedIsStatic` infrastructure as the lift point (research §3.3 + §3.10).
...
- [ ] **Replace committed `interpreter_mac_arm64` + tighten `verify.sh` check 5.** Only after D2-reborn lands and the new emit is at perf parity (or better) with the committed bridge for tree-walker workloads. The replacement is what makes `verify.sh` check 5 a TRUE fixed-point check (stage1 install → stage2 build → diff) instead of mere determinism.

### P2.6 — D2-reborn (NEW, 2026-05-18; gates committed-binary-replace)

Per the P2-stage2-regression-fix forensics this loop, the visible P2.5 gain is doubly blocked: (a) the committed bridge can't be replaced without (b) the new emit reaching tree-walker perf parity with the bridge. (b) requires re-emitting IJ-level static functions as direct Go functions instead of routing every call through `evalCall → FunctionCommand.Execute → executeFunc → eval(body)`.
...
- [ ] **Lift `resolvedIsStatic` into emit.** The resolver already annotates static defs (`analyzeIsStatic` walks the body and writes `resolvedIsStatic: true` when no nested `def`, no dynamic lookups, no global writes — research §3.3). Currently those annotations have zero readers in emit (research §3.3 confirms). Wire `functionDeclarationToGo` to detect static defs and emit a sibling `ij_<name>_impl(ctx, arg0, arg1, ...) Value` function alongside (or instead of) the AST `nkFuncDecl` node.
- [ ] **Direct call-site dispatch in `callExpressionToGo`.** When the callee is a `nkIdent` whose `resolvedKind` is the static-def the emit just wrote, emit `result := ij_<name>_impl(ctx, <args>)` instead of building an AST `nkCall` literal. Falls back to the AST path for indirect calls (`callee[name]`, `callee()` where callee is a value).
- [ ] **`NewStaticFunctionCommand` wrapping for storage paths.** When a static def is referenced by name as a value (e.g. assigned into a map), wrap `ij_<name>_impl` in `NewStaticFunctionCommand` so it can be stored as `Value{tag: tFunc}` and called via the regular evalCall path when needed. Both paths must produce identical results.
- [ ] **Drop dead `ij_*_impl` callers in OLD-bridge-emit territory.** The current `fix_app_go.py` injects `EqualsBool`/`AsValue` helpers that piggy-back on the rename bridge. Once D2-reborn ships and the committed bridge is replaced, those injections should be removed (research §3.5; tracked under P5).
- [ ] **Determinism gate.** D2 emission must be deterministic (top-down emit order — already required for the existing fixed-point check 5). Two consecutive `compile-local.sh interpreter.s` must produce byte-identical output.
- [ ] **Bench gate.** `selfhosted_interpreter.sh sample.s` under fresh stage2-as-bridge must be ≤ committed-bridge time (~1m17s). Drop-rule on D2-reborn = parity (not 1.3×); we are recovering lost perf, not adding new wins.
- [ ] **Replace committed binary + tighten verify.sh check 5 to true fixed-point.** Only after the bench gate passes.
