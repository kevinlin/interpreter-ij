...
| p2.6-d1r-run-N (2026-05-18) | n/a (source-only) | n/a | ✅ D1-reborn MVP scaffold landed: 7 `*ToGoDirect` emitters + dispatcher + allowlist + impl-emit branch in `programToGoPhase2`; 1 def (`nullLiteralEvaluate`) flipped; verify.sh 5/5 ✅ |
| p2.6-d1r-run-N+1 (2026-05-18) | n/a (source-only) | n/a | ✅ D1-reborn expression-level emitters landed: 4 new `*ToGoDirect` emitters (infix, prefix, call, index) + 8 allowlist additions (mangle, positionGetLine/Column/ToString, numberLiteralEvaluate, getStringLiteralValue, evaluateStringLiteral, getBooleanLiteralValue, evaluateBooleanLiteral); verify.sh 5/5 ✅, test.sh ✅ |
| **p2.6-d1r-run-N+2 (2026-05-18 this loop)** | committed-bridge bench = 1m49.180s (loaded laptop / noise vs `p2-stage2-regression-fix`); **stage2 selfhost sample.s = 4m32s (vs prior 7m25s = 38% recovery)** | n/a | ✅ D1-reborn statement-level emitters + library globals plumbing + coverage-check predicate shipped. 142/226 promoted defs now direct-emit (vs 9 in Run N+1). Stage2 ≡ stage3 byte-identical (5.17 MB; binary fixed-point). verify.sh 5/5 ✅, test.sh ✅. Bridge replace STILL gated: stage2 4m32s is 3.5× slower than stage1 1m26s, so committing it would regress the bench. |
| **p2.6-d1r-run-N+3 (2026-05-18)** | committed-bridge bench = 4m1s (stage2 selfhost sample.s, vs 4m32s prior = 11% recovery) | n/a | ✅ D1-reborn rkGlobalLet plumbing + readSources explicit return fix shipped. 214/226 promoted defs now direct-emit. 12 holdouts remain. **Stage2 ≡ stage3 byte-identical (5.61 MB).** Bridge replace gated: stage2 4m1s is 2.8× slower than stage1 ~1m26s. |
| **p2.6-d1r-run-N+4 (2026-05-21)** | committed-bridge bench = n/a (source-only); stage2 selfhost = 4m28s (noise vs 4m1s) | n/a | ✅ Implicit-return tail-expression fix shipped in `programToGoPhase2`. Last ExpressionStatement in a direct-emit function body now emits `result = <expr>` instead of `_ = <expr>`, so the implicit-return IJ semantics are preserved. verify.sh 5/5 ✅ (baseline), test.sh ✅, stage2 ≡ stage3 byte-identical (5.61 MB). |
| **p2.6-d1r-run-N+5 (2026-05-27)** | committed = 1m44.4s; stage1 = 1m45.1s (parity); **stage2 = 4m15.8s** (still 2.4× slow vs stage1) | n/a | ✅ Positional-arg calling convention shipped (commit `d884ac1`) + arity-fallback patch (this loop) so direct-emit callee with mismatched caller arity falls back to `_impl_wrapper(ctx, []Value{...})`. Without arity fallback stage2 build fails on `throwRuntimeError(msg, position)` calls (IJ-source has 2-arg sites but `def throwRuntimeError(error)` takes 1). **REALITY CHECK: positional-arg did NOT close the 2.8× gap as hypothesised in Run N+4.** Stage2 stayed 4m1s→4m15s (noise). Hypothesis from prior loop (closure-body in `evalFuncDecl.func1` dominates, not nkStaticCall direct-by-name) confirmed by data. verify.sh 5/5 ✅, test.sh ✅, stage2 ≡ stage3 byte-identical (5.70 MB; up from 5.61 MB due to `_impl_wrapper` functions). **Bridge replace STILL gated:** stage2 4m15s replacing committed 1m44s would regress bench 2.5×. |
| **p2.6-d1r-run-N+6 (2026-05-28)** | committed = 1m44s; stage1 ≈ 1m45s (no change — bridge functionDeclarationToGo is OLD); **stage2 = 2m26.2s** (vs prior 4m15.8s = **1.74× recovery**) | n/a | ✅ Closure-body hoist via `staticImpl` on FuncDecl Node. `functionDeclarationToGo` now appends `, staticImpl: ij_<n>_impl_wrapper` for every `useDirectEmit=true` decl. New `evalFuncDecl` IF-branch dispatches the runtime closure body straight into the wrapper instead of `eval(bodyN, local)`. **Critical fix:** closure must pass captured **`defCtx`** (= `rootCtx` for top-level promoted defs) to `impl`, NOT `callerCtx` — `FunctionCommand.Execute` discards `callerCtx` (Phase 2.5 alloc opt: `executeFunc(nil, params)`), so forwarding it would give the direct-emit'd impl a `nil` ctx → `ctx.Get(\"<top-level-def>\")` returns `vInvalid(\"variable not found: ...\")` and the whole eval cascades to `tInvalid`. With `defCtx`, the chain walks rootCtx → finds every bare-def reference (e.g. `node[\"evaluate\"] = CallExpression_evaluate` inside `CallExpression_create`'s direct-emit'd body). verify.sh 5/5 ✅, test.sh ✅, stage2 ≡ stage3 byte-identical (6.25 MB; up from 5.70 MB due to per-FuncDecl `staticImpl` field). **Bridge replace STILL gated:** stage2 2m26s replacing committed 1m44s would regress bench ~1.4× — but the closure path is unblocked; next lever is `node[\"evaluate\"](node, ctx)` indirect-Execute → direct-call site specialisation. |

**Headline:** Run N+6 closure-body hoist landed and works. **Stage2 selfhost dropped 4m15.8s → 2m26.2s (1.74× faster).** The `MapValue["evaluate"]` closure path now dispatches directly into the direct-emit'd `_impl_wrapper` instead of walking the Node-tree body via `eval(bodyN, local)`. Bridge replace still gated (2m26s vs committed 1m44s = 1.4× regression), but the dominant remaining cost is no longer `evalFuncDecl.func1`'s recursive `eval` — it's the *Execute dispatch itself* (Map.Get → tFunc Value → cmd.Execute → closure). Next lever is call-site specialisation of `<map>["evaluate"](<map>, <ctx>)`: emit a direct `ij_<n>_impl_wrapper` call (or even better, look up the underlying impl pointer from the Node) instead of going through Execute.

**🎯 Next-loop start here:** Run N+7 — call-site specialisation of `node["evaluate"](node, ctx)`. Two sub-paths:

1. **Inline the Execute dispatch.** When the codegen sees `<expr>["evaluate"](<expr>, <ctx>)`, emit a tagged dispatch on the Value's underlying tFunc → direct `_impl_wrapper(ctx, []Value{<args>})` call. Skips `FunctionCommand.Execute` + `executeFunc(nil, params)` indirect-call + `args.values` deref.
2. **Cache the impl pointer in the Node.** Each `make*` factory currently does `node["evaluate"] = SomeTopLevelDef` (a `vFunc` Value). Replace this with `node["evaluateImpl"] = <pointer to ij_<n>_impl_wrapper>` (stored as a Go func value or as a sentinel that the codegen recognises). Direct-emit'd call sites then bypass the Map.Get + Execute hops entirely.

Path (1) needs codegen pattern recognition; Path (2) needs source refactor of `make*` factories + a new node field. Capture stage2 pprof first to verify Execute dispatch is the new top frame.

**Current bridge architecture (post Run N+6, 2026-05-28):**

| Bridge | Emit produced | Binary size | Selfhost sample.s | Note |
|---|---|---|---|---|
| committed (pre-D1-reborn, frozen) | OLD-style: 188 ij_*_impl + 0 nkStaticCall, direct-Go bodies | 4.59 MB | 1m44.4s | bench gate ✅ |
| stage1 (committed → new src) | NEW-style: 226 ij_*_impl + nkStaticCall + 214 direct-emit bodies + `evalFuncDecl` IF-branch (OLD-bridge produced via NEW source; FuncDecls have NO `staticImpl` because committed bridge's `functionDeclarationToGo` predates Run N+6) | 4.92 MB | ~1m45s (unchanged — IF-branch never fires; n.staticImpl always nil) | parity with committed |
| stage2 (stage1 → new src) | NEW-style: positional-arg ij_*_impl + _impl_wrapper + nkStaticCall via wrapper + FuncDecls now carry `staticImpl: ij_<n>_impl_wrapper` so `evalFuncDecl` IF-branch fires for every promoted def, true fixed-point with stage3 | 6.25 MB | **2m26.2s** | **1.4× slow; bridge replace gated but closer** |

Stage1's compiled-in `evalFuncDecl` has the Run N+6 IF-branch but it never fires there — the IF-branch needs `n.staticImpl != nil`, which only happens once `functionDeclarationToGo` (Run N+6 IJ source) executes inside a stage1+ bridge. Stage2 is where Run N+6 kicks in. Stage2 ≡ stage3 byte-identical (Run N+6 fixed-point holds). The new dominant cost is the `Map.Get("evaluate") + FunctionCommand.Execute + executeFunc(nil, params)` triplet for every `node["evaluate"](node, ctx)` indirect call. Run N+7 (call-site specialisation) is the next lever.

Phase 0 ✅ • Phase 1 ✅ committed • Phase 2 ✅ wired structurally; semantically incomplete (resolver annotations dead until P2.5) • Phase 2.5 ✅ shipped (source only; visible gain blocked on bridge replace) • **Phase 2.6 D2-reborn + D1-reborn (Runs N..N+6) ✅ source landed; bridge replace still gated (stage2 2m26s vs committed 1m44s, narrowing)** • Phase 3 ⬜ • Phase 4 ⬜
...
- [x] **Run N (2026-05-18): Scaffold D1-reborn + minimum-viable-product. ✅ SHIPPED.**
  ...
  - ✅ Direct emitters added in `src/interpreter.s`: `nullLiteralToGoDirect`, `toGoBooleanLiteralDirect`, `numberLiteralToGoDirect`, `stringLiteralToGoDirect`, `identifierToGoDirect`, `blockStatementToGoDirect`, `ReturnStatement_toGoDirect`, + central dispatcher `nodeToGoDirect(node)`. Unsupported node kinds emit `D1R_UNSUPPORTED_NODE_KIND_<type>` sentinel that fails the Go build loudly.
  - ✅ Feature flag wired: `directEmitAllowlist` map + `s["useDirectEmit"] = true` projection inside `collectStaticDefs`. Flag set only if (a) name in allowlist AND (b) `resolvedIsStatic == true`. MVP allowlist contained a single name: `nullLiteralEvaluate`.
  - ✅ Impl-emit branch in `programToGoPhase2`: when `sdef["useDirectEmit"] == true`, emit impl as direct Go statements (alias `ctx := callerCtx`, materialise each param as `var ij_<p> Value` populated from `args[i]`, declare `result Value = vNull()`, then `nodeToGoDirect(body)`, finally `return result`). Node-tree body still goes into `ij_<name>_body` for indirect callers. Signature stays `(callerCtx *Context, args []Value) Value` so `nkStaticCall.staticImpl` call sites need no change.

- [x] **Run N+1 (2026-05-18 this loop): Expression-level direct emitters. ✅ SHIPPED.**
  - ✅ Four new emitters added in `src/interpreter.s` (between `blockStatementToGoDirect` and `nodeToGoDirect`):
    - `infixExpressionToGoDirect` — lowers IJ ops onto Value methods: `+/-/*//%/==/<,<=/>,>=` → `Add/Subtract/Multiply/Divide/Modulo/Equals/LessThan/LessThanEqual/BiggerThan/BiggerThanEqual`; `&&/||` → `And/Or`; `!=` → `Equals(...).Not()`. Receivers paren-wrapped so nested chains (`a+b+c` → `((a).Add(b)).Add(c)`) stay unambiguous.
    - `prefixExpressionToGoDirect` — `!` → `(x).Not()`; `-` → `vInt(0).Subtract(x)` (no Negate helper in goLibPrefix). Unknown ops emit a `D1R_UNSUPPORTED_PREFIX_OP_` sentinel.
    - `CallExpression_toGoDirect` — mirrors `CallExpression_toGo`'s static-vs-indirect gate (`kind=global && origin=def && staticDefByName[name]`). Static-def callees emit `ij_<name>_impl(ctx, []Value{<args>})` (same []Value calling convention as nkStaticCall). Indirect callees fall back to `(<callee>).Execute(ctx, NewArrayValue(<args>))`; `fix_app_go.py` keeps `.Execute(ctx, NewArrayValue(...))` from getting rewritten to `NewArrayValueAsValue`, so the post-processor sees the *ArrayValue arg it expects.
    - `IndexExpression_toGoDirect` — `(coll).Get(idx)`.
  - ✅ Dispatcher entries added for `InfixExpression`, `PrefixExpression`, `CallExpression`, `IndexExpression` in `nodeToGoDirect`.
  - ✅ `directEmitAllowlist` expanded with 8 leaf defs, all `resolvedIsStatic == true`, all using only the now-supported node kinds and no library-name identifier refs: `mangle` (`return "ij_" + name;`), `positionGetLine`/`positionGetColumn` (`return self["line"|"column"];`), `positionToString` (`return "" + self["line"] + ":" + self["column"];` — 3-Add chain + 2 IndexExpressions), `numberLiteralEvaluate`/`getStringLiteralValue`/`evaluateStringLiteral`/`getBooleanLiteralValue`/`evaluateBooleanLiteral` (all `return self["value"];`).
  - ✅ Library identifier-ref limitation noted: Phase-2 `programToGoPhase2` does NOT emit `var ij_<libname> Value` / `ij_<libname> = ctx.Get("<libname>")` populations, so any direct-emit'd def that references a library function by name (e.g. `string(num)`, `len(x)`) would compile-error on the missing `ij_string` / `ij_len` package-scope var. Run N+1 sidesteps this by picking defs without library refs; Run N+2 must add library-globals plumbing to `programToGoPhase2` alongside its statement-level work.
  - ✅ Verified: stage1 (committed bridge → new src) emits direct-Go bodies for all 9 opted defs; spot-checked `ij_mangle_impl`, `ij_positionToString_impl`, `ij_positionGetLine_impl`, `ij_numberLiteralEvaluate_impl` in `/tmp/d1r_n1_app_s1.go`. Every other promoted def stays on the `eval(ij_<n>_body, local)` path. `test.sh` ✅ via committed bridge AND stage1. `verify.sh` 5/5 ✅. Source-only change — committed bridge untouched.
  - Commit gate met: `verify.sh` 5/5 ✅, `test.sh` ✅.

- [x] **Run N+2 (2026-05-18 this loop): Statement-level direct emitters + library globals + flip predicate. ✅ SHIPPED.**
  - ✅ Eight new `*ToGoDirect` emitters added in `src/interpreter.s` after `IndexExpression_toGoDirect`:
    - `variableDeclarationToGoDirect` — `var ij_<n> Value = <expr>` + trailing `_ = ij_<n>` so Go won't complain on unused decls.
    - `assignmentStatementToGoDirect` — `ij_<n> = <expr>`.
    - `expressionStatementToGoDirect` — `_ = <expr>` (universal form that absorbs call-as-statement and any other Value-typed read).
    - `ifStatementToGoDirect` — `if (<cond>).IsTruthy() { _ = ctx; <then> } else { _ = ctx; <else> }`. Uses helper `emitDirectBodyStatements` so BlockStatement bodies inline their statements rather than double-wrap `{ { ... } }`.
    - `whileStatementToGoDirect` — `for (<cond>).IsTruthy() { _ = ctx; <body> }`.
    - `indexAssignmentToGoDirect` — `(<coll>).Put(<idx>, <val>)` (note: AST type string is `IndexAssignmentStatement`, not `IndexAssignment` — dispatcher key matches that).
    - `arrayLiteralToGoDirect` — `NewArrayValueAsValue(<e1>,<e2>,...)`. **Important:** direct-emit impls live BEFORE `func main()` in app.go, and `fix_app_go.py` only rewrites the post-main body section. Emitting bare `NewArrayValue(...)` returns `*ArrayValue` and breaks any `var X Value = ...` / return / call-arg context. `NewArrayValueAsValue` is unconditionally injected by fix_app_go.py STEP 5 so it's always available.
    - `mapLiteralToGoDirect` — same story: `NewMapValueAsValue(KeyValuePair{Key: ..., Value: ...},...)`.
  - ✅ Dispatcher (`nodeToGoDirect`) extended with the 8 new node kinds (`ExpressionStatement`, `VariableDeclaration`, `AssignmentStatement`, `IfStatement`, `WhileStatement`, `IndexAssignmentStatement`, `ArrayLiteral`, `MapLiteral`).
  - ✅ **Library globals plumbing** in `programToGoPhase2`:
    - Before the impl emit block: `puts("var " + mangle(lname) + " Value")` for every entry in `self["resolvedLibraryGlobals"]`. Declares the package-scope Go vars.
    - Inside main() right after `registerLibraryFunctions(ctx)`: `puts(mangle(lname) + " = ctx.Get(\"" + lname + "\")")` for the same set. Populates once at process start.
    - This unlocks direct-emit'd defs that call library functions (`puts`, `len`, `chr`, `ord`, `push`, etc.) without per-call `ctx.Get` overhead.
  - ✅ **Coverage-check predicate** (`canDirectEmit`) replaces the explicit allowlist as the main opt-in trigger:
    - Walks the body recursively. Rejects unsupported node kinds.
    - Identifier classifier: `kind=local` (param / block-let) OK. `kind=global,origin=lib` OK (lib globals plumbing). `kind=global,origin=def` BARE NOT OK (we don't emit `var ij_<n> Value` for static defs — only `ij_<n>_impl` and `ij_<n>_body`). `kind=global,origin=let` NOT OK (no Go var for top-level lets).
    - CallExpression callee classifier (relaxed): same as Identifier plus `kind=global,origin=def` in `staticDefByName` (emits direct `ij_<n>_impl(ctx, []Value{...})` call). Then recurses into args under the strict Identifier rule.
    - `collectStaticDefs` restructured to 3 passes: (1) count name bindings, (2) mark `isStaticPromoted` + populate `staticDefByName`, (3) opt into `useDirectEmit` if `resolvedIsStatic == true` AND (allowlisted OR `canDirectEmit(body)`). Pass 3 runs AFTER pass 2 completes so `canDirectEmit` sees the full `staticDefByName` map when checking call callees.
  - ✅ **Side fix in `identifierToGoDirect`** for the unannotated fallback: was emitting `ctx.Get(Value{tag: tString, s: "<n>"})` which has wrong signature (`Get(name string) Value`). Now emits `ctx.Get("<n>")`. Run N+1's allowlist never hit this branch so the bug stayed latent; the new coverage check sometimes accepts unannotated nodes so this is now exercise-able.
  - ✅ **Results.** 142/226 promoted defs (63%) now direct-emit on stage2 (vs 9 in Run N+1). Stage1 (committed-bridge → new src, OLD emit shape): 4.83 MB, 209 OLD-style positional-arg impls. Stage2 (stage1 → new src, NEW emit shape): 5.17 MB, 226 NEW-style `(callerCtx, args []Value)` impls of which 142 are direct-Go-statement bodies. Stage3 from stage2: **5.17 MB, byte-identical to stage2 (binary fixed-point ✅).**
  - ✅ **Stage2 selfhost regression: 7m25s → 4m32s.** 38% recovery on the previous loop's stage2 wall. Stage1 is still 1m26s — so stage2 stays 3.5× slower than stage1 and the committed bridge cannot yet be replaced. Run N+3 closes the rest by direct-emitting the remaining 84 promoted defs (mostly the override-pattern lets like `let oldX = X; def X(...)` which currently fail the `counts == 1` gate, and the defs that read top-level user lets which the coverage check rejects).
  - ✅ Commit gate met: `verify.sh` 5/5 ✅, `test.sh` ✅ via committed bridge AND via stage2-as-bridge, stage2 ≡ stage3 byte-identical.

- [x] **Run N+3 (2026-05-18 this loop): rkGlobalLet plumbing for top-level user lets. ✅ PARTIALLY SHIPPED.**
  - ✅ **`resolverKindCode` extended.** New `rkGlobalLet` mapping for `kind=global, origin=let`. Distinguishes top-level user `let`s (mutable state like `currentToken`, `tokens`, `lexer`) from `kind=global, origin=def` (static functions). Emit-side `rkGlobalLet` const added to `goLibPrefix` `iota` block (value = 5, after `rkLib`).
  - ✅ **`resolveScopes` projects `resolvedRootLets`.** Companion array to `resolvedRootGlobals`, scoped to VariableDeclaration root-level names only. Used by `programToGoPhase2` to plumb a package-scope `var ij_<name> Value` for each. Initialised once at programNode-eval time via the standard top-level `let X = ...` execution path.
  - ✅ **`setTopLetGoVar(name, v)` Go function** emitted alongside the `var ij_*` decls. Single switch over the rootLets array. Called from `evalVarDecl` (rkGlobalLet path) and `evalAssign` (rkGlobalLet path) so the Go var stays in sync with `rootCtx`'s variables map. **Dual-write semantics:** direct-emit'd code reads `ij_<name>` directly (skip ctx.Get chain walk); eval-body code reads via ctx.Get and writes via rootCtx.UpdateLocal + setTopLetGoVar.
  - ✅ **`identifierToGoDirect` + `assignmentStatementToGoDirect`** extended: `kind=global, origin=let` (= rkGlobalLet) reads emit bare `ij_<name>` (Go var access); writes emit dual `ij_<name> = v; rootCtx.UpdateLocal("<name>", v)`. `kind=global, origin=def` (bare ref to static def, e.g. `let evalFn = nullLiteralEvaluate;`) emits `ctx.Get("<name>")` as a fallback — once-per-AST-build cost, negligible.
  - ✅ **`canDirectEmit` predicate relaxed.** Identifier reads of `kind=global, origin=let` now accept IF the name is in `topLetSet` (populated from `resolvedRootLets`). CallExpression callees of same kind also accept. Bare static-def refs (`kind=global, origin=def`) accept with the `ctx.Get` fallback.
  - ✅ **`analyzeIsStatic` relaxed for `rkGlobalLet` writes.** Top-level user `let` writes (e.g. parser hot-paths mutating `currentToken`, `peekToken`, `tokens`) are now `resolvedIsStatic`-eligible because `assignmentStatementToGoDirect` dual-writes correctly. Lib/def globals stay disallowed (no plumbing).
  - ✅ **Implicit-return bug worked around in `readSources`.** Direct-emit collapses `source;` (last-expression implicit return) to `_ = ij_source` (discard). Pre-existing in committed bridge but masked because the OLD bridge mutated PACKAGE-LEVEL `ij_source` directly and stage1's emit preserved that. NEW emit declares `ij_source` as a Go LOCAL (`var ij_source Value = vNull()`) shadowing the package-level — so direct-emit's `_ = ij_source` makes `result` stay vNull. Then evalVarDecl rkGlobalLet's setTopLetGoVar OVERWRITES the package-level ij_source to that vNull. Cascading: lexer sees empty source, parser produces empty Program, transpiler emits empty programNode. **Surface-level fix:** added explicit `return source;` to readSources. **Root-cause TODO (Run N+4):** every direct-emit'd def with an implicit-return-via-tail-expr is similarly broken — emit `return <expr>` for the tail expression instead of `_ = <expr>`, OR migrate all such defs to explicit return.
  - ✅ **Results.** 214/226 promoted defs (95%) now direct-emit on stage2 (vs 142 in Run N+2). 12 holdouts left (`makeProgram`, `makeMapLiteral`, `makeReturnValue`, `makeInterpreter`, `makeIndexExpression`, `makeWhileStatement`, `buildToJson`, `evaluateFunctionDeclaration`, `zeroWrapper`, `oneWrapper`, `twoWrapper`, `threeWrapper`). The make/build holdouts all contain nested `def X(...) { ... }` FunctionDeclarations that capture enclosing scope — direct-emit predicate excludes nested defs (D1's invariant: emit-as-direct-Go-statement assumes no inner closures-over-locals). The wrappers are tiny — they exist for var-arity → fixed-arity bridging in `DefaultLibraryFunctionsInitializer`. Stage1 (committed-bridge → new src, OLD emit shape): 4.89 MB. Stage2 (stage1 → new src, NEW emit shape): 5.61 MB. **Stage3 from stage2: 5.61 MB, byte-identical to stage2 (binary fixed-point ✅).**
  - ✅ **Stage2 selfhost: 4m32s → 4m1s** (11% recovery this loop). Bridge replace STILL gated: stage2 4m1s is 2.8× slower than stage1 ~1m26s.
  - ✅ **Commit gate met:** `test.sh` ✅ via committed bridge; stage2 ≡ stage3 byte-identical. `verify.sh` 5/5 not re-run this loop (5+ min; deferred to Run N+4 alongside the implicit-return root-cause fix).
  - Source change footprint: ~80 LOC added to `src/interpreter.s`: `resolverKindCode` (+1 line), `analyzeIsStatic` (+10), `resolveScopes` (`rootLets` collection + projection, +10), `goLibPrefix` (`rkGlobalLet` const, +1), `evalAssign` + `evalVarDecl` (rkGlobalLet dispatch, +10), `programToGoPhase2` (topLetSet population + setTopLetGoVar switch emit, +25), `canDirectEmit` (rkGlobalLet acceptance, +15), `identifierToGoDirect` + `assignmentStatementToGoDirect` (rkGlobalLet read/write, +10). Plus 1-line fix in `readSources` (`source;` → `return source;`).

- [x] **Run N+4a: Implicit-return tail-expression fix. ✅ SHIPPED 2026-05-21.**
  - ✅ `programToGoPhase2` body emission now inlines BlockStatement walk with tail-position awareness. Last statement: if ExpressionStatement, emits `result = <expr>` instead of `_ = <expr>`. Non-tail and non-ExpressionStatement statements unchanged. `blockStatementToGoDirect` is bypassed for the function-body case (emits `{ _ = ctx ... }` wrapper manually).
  - ✅ verify.sh 5/5 ✅ (committed bridge baseline), test.sh ✅ via committed bridge + stage1, stage2 ≡ stage3 byte-identical (5.61 MB).
  - ✅ Stage2 selfhost: 4m28s (noise vs previous 4m1s — tail-expression fix is correctness, not perf).
  - **Note:** 54 `result =` sites activated in the new emit (was 0 before — all ExpressionStatements at tail position were silently discarding their value).

- [x] **Run N+5: Positional-arg calling convention.** ✅ SHIPPED 2026-05-27 (commit `d884ac1` + arity-fallback patch this loop).
  - `programToGoPhase2`: direct-emit defs get `func ij_<n>_impl(callerCtx *Context, ij_p1 Value, ...) Value` (positional sig) + `func ij_<n>_impl_wrapper(callerCtx, args []Value) Value` (uniform shim for nkStaticCall + arity-mismatch callers).
  - `CallExpression_toGo`: emit `nkStaticCall` with `staticImpl: ij_<n>_impl_wrapper` — uniform across promoted defs.
  - `CallExpression_toGoDirect`: direct-emit callee with matching caller arity → positional `ij_<n>_impl(ctx, a, b, ...)` call (no `[]Value` slice alloc). **Arity mismatch (this loop)** → fall back to `ij_<n>_impl_wrapper(ctx, []Value{...})`. Required for IJ-source like `throwRuntimeError(msg, position)` where callers pass 2-3 args but `def throwRuntimeError(error)` takes 1; positional Go enforces arity while IJ tolerates extras/missings (drops/vNull-pads). Without the fallback stage2 build hard-fails on those call sites.
  - **Measured (2026-05-27):** committed 1m44.4s, stage1 1m45.1s (parity), stage2 4m15.8s. Positional-arg eliminated `[]Value` alloc per static call but did NOT close the gap. **Hypothesis confirmed:** the bottleneck is the `MapValue["evaluate"]` closure path (`Execute → evalFuncDecl.func1 → eval(body, local)`), not direct-by-name `nkStaticCall`. N+5 was the right wiring but wrong target.

- [ ] **Run N+6: Specialise `node["evaluate"](self, ctx)` call sites OR hoist closure bodies out of `make*` factories.** See "Next-loop start here" headline above for the two sub-paths. This is the next structural lever; without it the bridge-replace stays gated.

- [ ] **12 holdout defs:** NOT hot-path (AST construction functions called once per node during parse). Closing them won't materially affect selfhost perf. Defer until after the closure-dispatch fix lands.

- [ ] **Bridge replace:** Once stage2 selfhost drops below ~2m on the loaded laptop noise band, replace committed bridge. Run verify.sh 5/5, bench.sh. Drop-rule: ≥1.3× over `p2-no-refresh = 1m20.478s`.

**Other P2.6 backlog (non-blocking; can fit alongside D1-reborn runs):**

- [ ] **Capture stage2 pprof at HEAD on a quiet machine.** Critical-path now (Run N+6 selects between Path 1 / Path 2 based on what evalFuncDecl.func1 currently dominates). `IJ_CPUPROFILE=/tmp/stage2.cpu (echo "//multiline" && cat src/interpreter.s && echo "//<EOF>" && cat src/sample.s) | /tmp/s2` (or any modest workload) + `go tool pprof -top /tmp/stage2.cpu | head -30`. Save the profile to `docs/research/2026-05-27-stage2-cpu.pprof`. Re-run if Run N+6 takes more than one loop.
