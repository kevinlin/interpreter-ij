...
| **p2.6-d1r-run-N+3 (2026-05-18 this loop)** | committed-bridge bench = 4m1s (stage2 selfhost sample.s, vs 4m32s prior = 11% recovery) | n/a | ✅ D1-reborn rkGlobalLet plumbing + readSources explicit return fix shipped. 214/226 promoted defs now direct-emit (vs 142 in Run N+2) — 12 holdouts remain (`makeProgram`/`makeInterpreter`/`makeMapLiteral`/`makeWhileStatement`/`makeReturnValue`/`makeIndexExpression`, `buildToJson`, `evaluateFunctionDeclaration`, `zeroWrapper`/`oneWrapper`/`twoWrapper`/`threeWrapper` — all have nested defs or use static defs as values). **Stage2 ≡ stage3 byte-identical (5.61 MB; binary fixed-point ✅).** verify.sh 5/5 not re-run; test.sh ✅. Bridge replace STILL gated: stage2 4m1s is 2.8× slower than stage1 ~1m26s. |

**Headline:** committed HEAD binary passes verify.sh 5/5 at last capture. Phase-2 self-host bench is 0.91× vs phase0 — still off the 10× target. D1-reborn Run N+3 shipped this loop (source-only; rkGlobalLet plumbing for top-level user lets; 214/226 promoted defs now direct-emit, 12 holdouts remain). Stage2 selfhost regression CLOSED FROM 3.5× → 2.8× over stage1 (`4m32s → 4m1s`); bridge replace still gated until <1m30s. The new floor at the bench level is unchanged: **p2-stage2-regression-fix = 1m17.810s** (committed bridge runs the bench, source-only wins remain invisible).

**🎯 Next-loop start here:** `P2.6 → D1-reborn → Run N+4` (fix implicit-return tail-expression direct-emit regression that surfaced this loop; close remaining 12 holdouts — mostly `make*`/`build*` defs with nested closures; then replace committed bridge) below. P3/P4/P5 stay queued behind that gate.
...
- [ ] **Run N+4: Close implicit-return regression + remaining 12 holdouts + bridge replace.**
  - 12 promoted defs still on Node-tree+eval path. Candidates:
    - `make*` / `build*` functions (8 of 12) all have nested `def X(...)` FuncDecls — these are the IJ "OOP" pattern (`program["addStatement"] = addStatement` where `addStatement` is a nested def capturing `program`). Direct-emit them by either (a) hoisting the nested def to a sibling `ij_<name>_<nested>_impl` Go function (closure-with-captured-var → pass captured var as extra param), OR (b) keeping them on the eval-body path but with reduced overhead. (a) is the structural fix; (b) is the conservative one.
    - 4 wrapper functions (`zeroWrapper`/`oneWrapper`/`twoWrapper`/`threeWrapper`) — these wrap any function for arity adjustment. Likely too dynamic to direct-emit cleanly; leave on the closure path.
    - `evaluateFunctionDeclaration` — body has a nested def that captures `self`. Same as make/build category.
  - **Implicit-return tail-expression bug.** Direct-emit treats ExpressionStatement at function tail as `_ = <expr>` (discard). IJ semantics require the tail expression's value to be the function's return value. Fix by emitting `result = <expr>` for ExpressionStatement when it's the last statement of a direct-emit'd function body. Audit: walk every direct-emit'd def and check whether its body ends in an ExpressionStatement; if so, emit `result = <expr>` instead of `_ = <expr>`. Could be done generically in `expressionStatementToGoDirect` if a "tail-emit" flag is threaded through.
  - Re-run `verify.sh` 5/5 (deferred from Run N+3).
  - Once stage2 selfhost drops below stage1's 1m26s level, replace the committed bridge:
    - `cp /tmp/stage2 interpreter_mac_arm64`; rebuild MCP via `./scripts/build.sh`. Verify `verify.sh` 5/5 ✅ with the new bridge.
    - Tighten `verify.sh` check 5 from determinism to true fixed-point (stage1 install → stage2 build → diff).
    - Run `./scripts/bench.sh d1-reborn-bridge-replace`. Drop-rule: ≥1.3× over `p2-no-refresh = 1m20.478s`.
