...
|---|---|---|---|
| **committed binary** (`interpreter_mac_arm64`, now `4c42e04b…` after the P-VM.2 replace; `255e274e…` was P-VM.1, `fa1fe55…` was the 2026-05-29 original) | ~120s pinned (2026-06-11 box state; see P-VM.1 bench note — only same-session comparisons are valid) | NEW emit + dormant Go-side VM (`IJ_VM=1` opt-in) | **IS the true fixed point of current source at every commit since 2026-05-29 (verify.sh check 5); re-replaced each loop that touches the prelude** |
...
Phase status:
 ...
 • P-VM.3 ⬜ NEXT (flip default to VM + re-establish fixed point)
 • ~~P3~~ deprioritised
...
- [ ] **P-VM.3 — flip default to VM + re-establish the true fixed point** (committed→s1→s2, `cmp s2 s3`; mirror the P-C bridge-replace procedure). The delicate step — never flip before 5/5 under the flag. Pre-flight notes (2026-06-11):
  - All verify.sh checks already pass WITH `IJ_VM=1` manually; the flip changes `main()`'s env-gate default (or removes it). Decide: keep `IJ_VM=0` as an opt-out during P-VM.4, or delete the gate.
  - MCP (`mcp_eval.s` overlay) overrides `gets`/`puts` via the override idiom — those are top-level redefinitions in the CONCATENATED source, so its program chunk will compile func chunks for them. `vm_difftest.sh` does not cover MCP; **add an MCP differential** (mirror verify check 4 with `IJ_VM=1`) before flipping.
  - The 3-layer selfhost perf under the VM is NOT yet the ≥8× win — that's P-VM.4 (IJ-side mirror). The flip itself is correctness/infrastructure.
