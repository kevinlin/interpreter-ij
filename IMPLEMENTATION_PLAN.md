...
- [ ] **Task 2.5.6: Gate `evalBlock` Context allocation on `hasLocals`.** `blockStatementToGo` (`1100`) projects `hasLocals: true` only when `resolvedLocals` non-empty; `evalBlock` (`5275`) skips `NewContext(ctx)` when `hasLocals == false`. Eliminates per-iteration Context alloc inside `while` loops with no `let` (the `sample.s` shape). **Also unblocks the `rkParam`/`rkLocal` GetLocal fast path in `evalIdent`** (collapsing per-block ctxs into the function ctx makes the function-local map probe correct).
