...
| **p2.6-d1r-run-N+4 (2026-05-21 this loop)** | committed-bridge bench = n/a (source-only); stage2 selfhost = 4m28s (noise vs 4m1s) | n/a | ✅ Implicit-return tail-expression fix shipped in `programToGoPhase2`. Last ExpressionStatement in a direct-emit function body now emits `result = <expr>` instead of `_ = <expr>`, so the implicit-return IJ semantics are preserved. verify.sh 5/5 ✅ (baseline), test.sh ✅, stage2 ≡ stage3 byte-identical (5.61 MB). **Positional-arg calling convention (the key to closing the 2.8× gap) investigated but BLOCKED on committed-bridge incompatibility:** the committed bridge's compiled-in transpile functions (from pre-Phase-2 era) cannot emit wrappers or positional-arg calls; source changes to `CallExpression_toGo` / `CallExpression_toGoDirect` / wrapper emission break the committed bridge build. **Next step: replace the committed bridge with a stage1 bootstrap, then apply positional-arg calling convention from the new bridge.** |

**Headline:** Run N+4 tail-expression fix shipped (correctness). Positional-arg calling convention identified as the structural fix for the 2.8× selfhost gap, but it requires a stage1 bootstrap bridge first (committed bridge's frozen transpile functions don't support wrapper/positional-arg emit). Path forward: replace committed bridge → apply positional-arg conv → measure stage2 selfhost → if < stage1, keep new bridge.

**🎯 Next-loop start here:** Replace committed bridge with stage1 bootstrap, then apply positional-arg calling convention changes to `programToGoPhase2`, `CallExpression_toGoDirect`, and `CallExpression_toGo`. These changes eliminate `[]Value` slice alloc per static call (the dominant overhead vs the OLD bridge's positional-arg D2 fast path).
...
- [ ] **Run N+4b: Positional-arg calling convention.** The committed bridge's compiled-in transpile functions are from pre-Phase-2 era and cannot emit wrappers or positional-arg calls. To apply the positional-arg change (eliminating `[]Value` slice alloc per static call), we must first replace the committed bridge with a stage1 bootstrap. Then from stage1, the positional-arg changes to `programToGoPhase2`, `CallExpression_toGoDirect`, and `CallExpression_toGo` take effect via stage1's `programToGoPhase2` (which IS called by stage1's transpile path).

  Implement after bridge replace:
  - `programToGoPhase2`: emit positional-arg impls for direct-emit defs (`func ij_<n>_impl(callerCtx *Context, ij_p1 Value, ...) Value`) + wrappers for nkStaticCall compat.
  - `CallExpression_toGoDirect`: emit positional-arg calls for direct-emit callees, `[]Value` calls for non-direct-emit static defs.
  - `CallExpression_toGo`: emit `nkStaticCall` with `_impl_wrapper` (uniform for all promoted defs).

- [ ] **12 holdout defs:** NOT hot-path (AST construction functions called once per node during parse). Closing them won't materially affect selfhost perf. Defer until after positional-arg calling convention lands.

- [ ] **Bridge replace:** Once stage2 selfhost drops below stage1's ~1m26s level, replace committed bridge. Run verify.sh 5/5, bench.sh. Drop-rule: ≥1.3× over `p2-no-refresh`.
