...
**Next-loop target = P2 stage2-regression fix THEN P3:** P2.5 source work shipped 2026-05-17 (commits `6ca08e9..5bf147a`). Bench gain not visible because selfhosted bench routes through the COMMITTED binary which is the pre-P2.5 bridge. Replacing the committed binary requires fixing the P2 stage2 multi-statement regression — at HEAD a fresh stage2 build aborts after the first top-level statement in user-supplied scripts (e.g. `puts(1);puts(2);` → prints only `1`). All P2.5 emit + runtime changes are correct on stage1 (`test.sh` ✅ + `verify.sh` 5/5 ✅) and would harvest gain on stage2 once the bridge is replaced.

Phase 0 ✅ • Phase 1 ✅ committed • Phase 2 ✅ wired structurally; semantically incomplete (resolver annotations dead) • **Phase 2.5 ✅ shipped (source only; visible gain blocked on P2 bridge replace)** • Phase 3 ⬜ • Phase 4 ⬜
