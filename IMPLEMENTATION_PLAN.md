...
**🎯 Next-loop start here:** `P2.6 → D1-reborn → Run N` below. Every other P2.5/P2.6 source-level win is invisible to `bench.sh` until the committed bridge is replaced, and the bridge can't be replaced until D1-reborn closes the 5× stage2 perf regression. P3/P4/P5 stay queued behind that gate.
...
- [ ] **Run N (next): Scaffold D1-reborn + minimum-viable-product.**
  - Capture the OLD-bridge emit template to a reference file (rebuild stage1 via `compile-local.sh src/interpreter.s /tmp/d1r/stage1` and save the pre-`fix_app_go.py` `app.go` to `docs/research/2026-05-18-d1-reborn-emit-template.md` or similar — extract the 200-line per-statement mapping while it's fresh).
  - Add leaf direct emitters: `literalsToGoDirect` + `identifierToGoDirect`. These have no recursion and exercise the whole direct-emit pipeline end-to-end. Wire them behind a feature flag (e.g. `staticDefByName[name]["useDirectEmit"] = true` for opt-in per-def).
  - Pick **one** promoted static def whose body uses only literals + identifiers (search parser tag tests like `isDigit(ch)` / `isAlpha(ch)`) and flip it to direct emit. `compile-local.sh` + `test.sh` + fib25 micro-bench. If stage2 wall on fib25 drops measurably, the architectural fix is real and the next loops can scale up.
  - Commit gate: `verify.sh` 5/5 ✅, `test.sh` ✅. No bench drop-rule yet — proof of concept only.
