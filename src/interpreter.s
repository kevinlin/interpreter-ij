...
//     with slot-indexed frame locals (params + function-scope lets). Bail
//     rules keep tree-walk semantics exact: no nested defs, no upvalues, no
//     lets in nested blocks (block-scoped shadowing), no unannotated global
//     writes, no array/map/index/staticCall nodes (P-VM.2 scope).
//   - promoted defs (staticImpl != nil) stay on the evalFuncDecl fast branch
//     via the escape -- direct Go bodies are already faster than the VM.
...
puts("ji2 := c.emit(vmOpJumpIfInvDrop, 0, 0)");
...
puts("c.emit(vmOpConst, c.constIdx(Value{tag: tDouble, d: n.dVal}), 0)");
...
puts("ji := c.emit(vmOpJumpIfInvDrop, 0, 0)");
puts("jf := c.emit(vmOpJumpIfFalse, 0, 0)");
puts("c.compileStmt(n.body)");
puts("if !c.ok { return }");
puts("c.emit(vmOpStep, 0, 0)");
...
puts("if n.staticImpl != nil { c.ok = false; return }");
...
puts("c.emit(vmOpStep, 0, 0)");
puts("}");
puts('if vmDebug { fmt.Fprintln(os.Stderr, "[vm] program stmts:", len(n.list), "escaped:", len(ch.nodes), "funcChunks:", len(ch.fns)) }');
puts("return ch");
puts("}");
...
puts("res := callee.cmd.Execute(ctx, av)");
puts("vmStack[vmSP] = res");
puts("vmSP++");
